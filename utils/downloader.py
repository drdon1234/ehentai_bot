...
class Downloader:
    ...
    async def download_image_with_fixed_number(self, session: aiohttp.ClientSession, img_url: str, image_number: int) -> bool:
        proxy_conf = self.config['request'].get('proxy', {})
        cookies = self.config['request']['cookies'] if self.config['request']['website'] == 'exhentai' else None
    ...
    async def retry_failed_downloads(self, ctx: EventContext, session: aiohttp.ClientSession, failed_items: list) -> list:
        if not failed_items:
            return []
